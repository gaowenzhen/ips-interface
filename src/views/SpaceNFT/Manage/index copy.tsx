...
import { IPSMetAverseRegisterAddress, IPSNftAddress, ZERO_ADDRESS } from "../../../constants"
...
export default function Manage() {
	...
	const createMeta = useCallback(
		async ({ tokenId, bindAddress }: MetaNftType) => {
			if (bindAddress !== ZERO_ADDRESS) return ipsTips(t`Metauniverse has been created`, { variant: "warning" });
			if (!!ipsFactoryContract) {
				const info = await ipsFactoryContract.createMetaverse(
					[
						"SANDBOX",
						"sandbox",
						BigNumber.from('100'), // 拍卖数量
						BigNumber.from('3'),  // 钱包mint数量
						BigNumber.from('200'),  //  钱包发行总量
						tokenId.toString(),
						IPSNftAddress
					]
				)
		...
		}, [ipsFactoryContract, IPSNftAddress])
	...
	useEffect(() => {
		if (IPSNFT?.ipsNft && ipsCoordinateSystem && ipsCns && ipsBaseCoordinateBindContract) {
			...
			IPSNFT?.ipsNft.filter((item:any) => item.isBinded).forEach(async ({ tokenId, bindAddress }) => {
	...
	}, [IPSNFT, ipsCoordinateSystem, ipsCns, ipsBaseCoordinateBindContract])

	// 获取nft
	// const getNFT = useCallback(
	// 	async () => {
	// 		if (!!ipsNftContract && !!ipsMetAverseRegisterContract && !!ipsCoordinateSystem) {
	...
	// 	},
	// 	[ipsNftContract, account, ipsMetAverseRegisterContract, ipsCoordinateSystem]
	// )
							...
							<ItemDetail>
								...
								<Options>{
									item.metaverse === ZERO_ADDRESS ? <Price onClick={(e:any) => {
										...
										<Fragment><Price>Coord:</Price> <Price>[{item.coord.join(", ")}]</Price></Fragment>}
								</Options>
								{
									item.cnsName ?
										<Fragment>
